...
csv_path = r'D:\Postdoc\Paper 12\Datasets\Coal Shandong\Models\training_log_KD.csv'
...
#         teacher_train_features.append(t_feat.cpu())
#         student_train_features.append(s_feat.cpu())
#         train_labels.append(labels.cpu())
#
# teacher_train_features = torch.cat(teacher_train_features).numpy()
# student_train_features = torch.cat(student_train_features).numpy()
# train_labels = torch.cat(train_labels).numpy()
#
# # t-SNE降维
# tsne = TSNE(n_components=2, random_state=42, perplexity=30, n_iter=300)
# t_tsne = tsne.fit_transform(teacher_train_features)
# s_tsne = tsne.fit_transform(student_train_features)
#
# # 绘制对比图
...
# # 教师模型特征空间
# plt.subplot(121)
# sc = plt.scatter(t_tsne[:,0], t_tsne[:,1], c=train_labels,
#                 cmap='viridis', alpha=0.6, s=15)
# plt.title("Teacher Feature Space (LIBS) - Training Set", fontsize=12)
...
# # 学生模型特征空间
# plt.subplot(122)
# sc = plt.scatter(s_tsne[:,0], s_tsne[:,1], c=train_labels,
#                 cmap='viridis', alpha=0.6, s=15)
# plt.title("Student Feature Space (NIR) - Training Set", fontsize=12)
# plt.xlabel("t-SNE 1", fontsize=10)
# plt.ylabel("t-SNE 2", fontsize=10)
# cbar = plt.colorbar(sc)
# cbar.set_label('Target Value', fontsize=10)
