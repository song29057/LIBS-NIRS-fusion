...
class FusionModel(nn.Module):
    ...
    def forward(self, nir_data, libs_data):
        ...
        output_nir = self.self_attention(nir_feat)
        output_libs = self.self_attention(libs_feat)

        output_nir = self.ln1(output_nir)
        output_nir = self.fc1(output_nir)
        output_nir = self.rl1(output_nir) + nir_feat

        output_libs = self.ln2(output_libs)
        output_libs = self.fc2(output_libs)
        output_libs = self.rl2(output_libs) + libs_feat

        combined_feat = torch.cat([output_nir, output_libs], dim=-1)  # 拼接特征
        combined_feat = self.ln0(combined_feat)
        gate = self.gate_fc(combined_feat)
        gate = self.sigmoid(gate)  # 计算门控权重

        fused_feat = gate * output_nir + (1 - gate) * output_libs  # 加权融合
...
best_model_path = r'D:\Postdoc\Paper 12\Datasets\Coal Shandong\Models\FeatureFusion_model.pth'
train_losses = []
val_losses = []
best_epoch = 0
...
csv_path = r'D:\Postdoc\Paper 12\Datasets\Coal Shandong\Models\training_log_FeatureFusion.csv'
...
file_path = r"D:\Postdoc\Paper 12\Datasets\Coal Shandong\Prediction\Fusion\FeatureFusion_val.csv"
...
file_path = r"D:\Postdoc\Paper 12\Datasets\Coal Shandong\Prediction\Fusion\FeatureFusion_test.csv"
